...
候选镜像先在本地 tag 下完成 E2E，之后才推送 GHCR。取得 digest 后，Release bundle 固定到：
...
其中包含 `source_commit`。正式 Release workflow 必须同时输入这个 40 位 SHA，并且它必须等于本次 workflow 的 `GITHUB_SHA`。只要代码之后又发生任何变化，就必须重新执行 Live Canary，旧证据不能用于放行新提交。
