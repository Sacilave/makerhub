...
RELEASE_PATTERN = re.compile(r"^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)-h([1-9]\d*)$")
...
def _check_versions(root: Path, tag: str) -> list[str]:
    ...
    try:
        ...
        return [f"RELEASE_VERSION could not be read: {exc}"]

    if not SEMVER_PATTERN.fullmatch(version):
        errors.append("VERSION must use X.Y.Z semantic version format")
    match = RELEASE_PATTERN.fullmatch(release_version)
    if not match:
        errors.append("RELEASE_VERSION must use X.Y.Z-hN format")
    elif release_version.split("-h", 1)[0] != version:
        errors.append(f"RELEASE_VERSION base {release_version.split('-h', 1)[0]} does not match VERSION {version}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check MakerHub base/release version consistency.")
    ...
    version = (root / "VERSION").read_text(encoding="utf-8").strip()
    release_version = (root / "RELEASE_VERSION").read_text(encoding="utf-8").strip()
    print(f"release version check passed: base={version}, release=v{release_version}")
    return 0
