...
def write_common(target:Path,image:str,version:str)->None:
    target.mkdir(parents=True,exist_ok=True); (target/'compose.yaml').write_text(render_compose(image),encoding='utf-8'); (target/'VERSION.txt').write_text(f'MakerHub release: {version}\nContainer image: {image}\n',encoding='utf-8'); (target/'README.md').write_text(f'''# MakerHub {version}\n\n这个包是 MakerHub 的可运行 Release，不需要克隆源码。\n\n## 前置条件\n\n- 64 位 x86-64 / amd64 机器\n- Docker Engine + Docker Compose v2\n- Windows 使用 Docker Desktop（WSL2 后端）\n\n## 首次启动\n\nWindows：`.\\makerhub.ps1 start`\n\nLinux：`./makerhub.sh start`\n\n启动器会自动生成本地数据库密码、CloakBrowser Token 和 AES-256 状态加密密钥，然后拉取本 Release 对应的不可变容器镜像。\n\n默认地址：http://127.0.0.1:9042\n\n常用命令：start / stop / restart / status / logs / doctor / password / update\n''',encoding='utf-8')
def build_windows(workdir:Path,image:str,version:str)->Path:
    target=workdir/WINDOWS_NAME; write_common(target,image,version); shutil.copy2(PACKAGING/'windows-amd64'/'makerhub.ps1',target/'makerhub.ps1'); shutil.copy2(PACKAGING/'windows-amd64'/'MakerHub.cmd',target/'MakerHub.cmd'); archive=DIST/f'{WINDOWS_NAME}.zip'
    with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as zf:
        for p in sorted(target.rglob('*')):
            if p.is_file(): zf.write(p,p.relative_to(workdir).as_posix())
    return archive
def build_linux(workdir:Path,image:str,version:str)->Path:
    target=workdir/LINUX_NAME; write_common(target,image,version); launcher=target/'makerhub.sh'; shutil.copy2(PACKAGING/'linux-amd64'/'makerhub.sh',launcher); launcher.chmod(launcher.stat().st_mode|stat.S_IXUSR|stat.S_IXGRP|stat.S_IXOTH); archive=DIST/f'{LINUX_NAME}.tar.gz'
...
def main()->int:
    parser=argparse.ArgumentParser(); parser.add_argument('--image',required=True); parser.add_argument('--release-version',default=(ROOT/'VERSION').read_text().strip()); a=parser.parse_args(); image=a.image.strip(); version=a.release_version.strip(); DIST.mkdir(parents=True,exist_ok=True)
    ...
    with tempfile.TemporaryDirectory(prefix='makerhub-release-') as temp:
        w=build_windows(Path(temp),image,version); l=build_linux(Path(temp),image,version); validate_archives(w,l,image)
    assets=[w,l]; (DIST/'SHA256SUMS').write_text('\n'.join(f'{sha256(p)}  {p.name}' for p in assets)+'\n'); manifest={'version':version,'image':image,'maintainer':{'name':'Sacilave','email':'sacilave@gmail.com'},'assets':{p.name:{'sha256':sha256(p),'bytes':p.stat().st_size} for p in assets}}; (DIST/'release-manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n'); print(json.dumps(manifest,ensure_ascii=False,indent=2)); return 0
